...
public class PlayListViewActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        progressDialog = new ProgressDialog(PlayListViewActivity.this, R.style.progressbart);
      ...
      /*  RecyclerView playlist = findViewById(R.id.playlist_rc);
        ...
        RecyclerView playlist = findViewById(R.id.playlist_rc);
        llm = new LinearLayoutManager(PlayListViewActivity.this);
        playlist.setLayoutManager(llm);
        playlist.setHasFixedSize(false);
        playlist.setNestedScrollingEnabled(false);
        final MusicAdapter adapter = new MusicAdapter(schedule.getPlaylist(), new ArrayList<PlayList>(), 2);
        ...
        adapter.setOnMenuClickListener(new MusicAdapter.OnMenuClickListener() {
            ...
            public void onItemClick(View itemView, final int position) {
                ...
                final PopupMenu popup = new PopupMenu(PlayListViewActivity.this, itemView);
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        ...
                        if (item.getTitle().toString().equals(getResources().getString(R.string.saveToCache))) {
                            if (hasPermissions()) {
                                ...
                                Toast.makeText(PlayListViewActivity.this, "Для сохранения, нужно разрешение на чтение и запись в память.", Toast.LENGTH_SHORT).show();
    ...
    void loadFullMusic() {
        if (!PlayListViewActivity.this.isFinishing()) {
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, final Response response) throws IOException {
                ...
                if (respons.contains("<div class=\"audioPlaylist__footer\">")) {
                    tmp1 = AudioUtil.pars("<div class=\"audioPlaylist__footer\">", respons, "</div>");
                ...
                title = AudioUtil.pars("<div class=\"audioPlaylist__title\">", respons, "</div>");
                String tmpf = AudioUtil.pars("<div class=\"audioPlaylist__subtitle\">", respons, "</div>");
                if (tmpf.contains("<a href=\"")) {
                    tmpf = AudioUtil.pars(">", tmpf, "</a>");
                }
                ...
                if (respons.contains("<div class=\"audioPlaylist__genres\"")) {
                    genr = AudioUtil.pars("<div class=\"audioPlaylist__genres\">", respons, "</div>");
                ...
                if (respons.contains("<div class=\"audioPlaylist__desc\">")) {
                    tmpf = AudioUtil.pars("<div class=\"audioPlaylist__desc\">", respons, "</div>");
                } else {
                    tmpf = "";
                }
                ...
                while (respons.contains("<div id=\"audio")) {
                    String tmp = "<div id=\"audio" + AudioUtil.pars("<div id=\"audio", respons, "<td class=\"aic_progress_wrap\">");

                    respons = respons.substring(respons.indexOf("<div id=\"audio") + 14);


                    String dataid = AudioUtil.pars("<div id=\"", tmp, "\"");
                    String artist = AudioUtil.pars("<span class=\"ai_artist\">", tmp, "</span>");
                    String mustitle = AudioUtil.pars("<span class=\"ai_title\">", tmp, "</span>");
                    if (mustitle.contains("ai_explicit")) {
                        mustitle = mustitle.substring(0, mustitle.indexOf("<div class=\"ai_explicit"));
                        mustitle += " Ⓔ";
                    }
                    String url = AudioUtil.pars("<input type=\"hidden\" value=\"", tmp, "\">");
                    String img;
                    if (tmp.contains("background-image:url(")) {
                        img = AudioUtil.pars("background-image:url(", tmp, ")\"");
                    } else {
                        img = "none";
                    }
                ...
                PlayListViewActivity.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ...
                        Picasso.get()
                                .load(AudioUtil.pars("url('", playList.getItemCover(), "')"))
                        ...
                        name_playlist.setText(AudioUtil.normalizeString(title));
                        name_sub.setText(AudioUtil.normalizeString(subtitle));
                        time_update.setText(AudioUtil.normalizeString(stats));
                        info_title.setText(AudioUtil.normalizeString(desc));
                        name_genres.setText(AudioUtil.normalizeString(genres));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.action_download) {
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(PlayListViewActivity.this, R.style.AlertDialog);
        ...
        if (allowed) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                builder.setNegativeButton(getResources().getString(R.string.finish), new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        Toast.makeText(PlayListViewActivity.this, "Аудиозапись не будет сохранена.", Toast.LENGTH_SHORT).show();
                    }
                });
                builder.show();
            }
